...
export class DashboardService {
  ...
  async getEstadisticasPracticas() {
    ...
    // Estudiantes en práctica por escuela
    ...
    const escuelas = await this.prisma.escuela.findMany({
      where: {
        id: {
          in: estudiantesPorEscuela.map((e) => e.escuela_id),
        },
      },
      select: {
        id: true,
    ...
    // Prácticas por mes
    const practicasPorMes = await this.prisma.$queryRaw`
      SELECT 
  ...
  async getEstadisticasTesis() {
    ...
    // Tesis por escuela
    const tesisPorEscuela = await this.prisma.$queryRaw`
      SELECT 
        e.nombre as escuela,
        COUNT(t.id) as total
      FROM tesis t
    ...
    // Tesis por año
    const tesisPorAnio = await this.prisma.$queryRaw`
      SELECT 
    ...
    return {
      ...
      tesis_por_escuela: tesisPorEscuela,
      tesis_por_anio: tesisPorAnio,
    };
  }

  async getEstadisticasEmpresas() {
    // Empresas con convenio vs sin convenio
    ...
    const empresas = await this.prisma.empresa.findMany({
      where: {
        id: {
          in: topEmpresas.map((e) => e.empresa_id),
        },
      },
      select: {
        id: true,
  ...
  async getIndicadoresRendimiento() {
    ...
    // Promedio de duración de tesis (en días)
