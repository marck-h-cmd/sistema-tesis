...
export default function OfertaDetailPage() {
  ...
  console.log("ofertad_id ", id);
  // Convert id to integer safely
  const ofertaId = id ? parseInt(id as string, 10) : null;
  console.log("ofertaId ", ofertaId);

  const { data: oferta, isLoading } = useQuery({
    queryKey: ['oferta', ofertaId],
    queryFn: () => ofertasApi.getOne(ofertaId as number).then(res => res.data.data),
    enabled: !!ofertaId && !isNaN(ofertaId),
  });

  const handlePostular = async () => {
    if (!ofertaId || isNaN(ofertaId)) {
    ...
    try {
      await ofertasApi.postular(ofertaId, {
